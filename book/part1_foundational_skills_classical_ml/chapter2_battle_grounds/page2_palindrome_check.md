...
# ⚔️ Kata 2: Palindrome Check

A palindrome reads the same forwards and backwards. "racecar", "madam", "level" — all palindromes.
...
- **Ignore punctuation** — `"A man a plan a canal Panama"` is a palindrome
...
> **Hint:** Clean the string first (lowercase, remove non-letters), then compare it to its reverse.

### Initial Code

```python
def is_palindrome(s):
    # Step 1: clean — lowercase and keep only letters
    cleaned = 
