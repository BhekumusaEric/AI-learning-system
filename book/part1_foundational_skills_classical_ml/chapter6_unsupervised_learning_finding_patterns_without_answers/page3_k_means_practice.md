...
# 1. Create K-Means model with 5 clusters
kmeans = 

# 2. Fit the model to the customer data
# (This assigns each customer to a cluster)

# 3. Get the cluster labels for each customer
cluster_labels = 
...
def check_clustering():
    return cluster_labels[:10], cluster_centers.shape
```
