...
import { BookOpen, CheckCircle2, UserPlus, LogIn } from 'lucide-react';
import Link from 'next/link';

type State = 'idle' | 'loading' | 'already_registered' | 'success' | 'error';

export default function DipRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ full_name: fullName.trim(), email: email.trim(), platform: 'dip' }),
      });
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-foreground">
        <div className="w-full max-w-md bg-secondary border border-border-subtle rounded-2xl p-8 shadow-2xl text-center">
          ...
          <Link href="/dip/login" className="w-full flex items-center justify-center gap-2 bg-accent text-black font-bold py-3 rounded-lg hover:bg-accent/90 transition-all">
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-foreground">
      <div className="w-full max-w-md bg-secondary border border-border-subtle rounded-2xl p-8 shadow-2xl">
        <div className="flex flex-col items-center mb-8">
          <div className="bg-accent/20 p-4 rounded-2xl mb-4"><BookOpen className="w-10 h-10 text-accent" /></div>
        ...
        <p className="text-center text-secondary-text text-xs mt-6">
          ...
          <Link href="/dip/login" className="text-accent hover:underline">Log in here</Link>
        </p>
      </div>
    </div>
  );
