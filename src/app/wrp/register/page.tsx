...
import { Briefcase, CheckCircle2, UserPlus, LogIn } from 'lucide-react';
import Link from 'next/link';

type State = 'idle' | 'loading' | 'success' | 'error';

export default function WrpRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ full_name: fullName.trim(), email: email.trim(), platform: 'wrp' }),
      });
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-foreground">
        <div className="w-full max-w-md bg-secondary border border-border-subtle rounded-2xl p-8 shadow-2xl text-center">
          ...
          <p className="text-secondary-text text-sm mb-6">Welcome, <span className="text-foreground font-semibold">{result.full_name}</span>!</p>
          ...
          {result.emailSent
            ? <p className="text-accent text-xs mb-6">✓ Credentials sent to your email.</p>
            : <p className="text-warning text-xs mb-6">⚠ Save these credentials — they won&apos;t be shown again.</p>}
          <Link href="/wrp/login" className="w-full flex items-center justify-center gap-2 bg-accent text-black font-bold py-3 rounded-lg hover:bg-accent/90 transition-all">
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-foreground">
      <div className="w-full max-w-md bg-secondary border border-border-subtle rounded-2xl p-8 shadow-2xl">
        <div className="flex flex-col items-center mb-8">
          <div className="bg-accent/20 p-4 rounded-2xl mb-4"><Briefcase className="w-10 h-10 text-accent" /></div>
          ...
          <p className="text-secondary-text text-center text-sm">Create your account to get started.</p>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)}
              ...
              className="w-full bg-background border border-border-subtle rounded-lg px-4 py-3 text-foreground focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all" required />
          </div>
          <div>
          ...
          {state === 'error' && <p className="text-error text-sm bg-error/10 border border-error/20 rounded-lg px-4 py-2">{errorMsg}</p>}
          <button type="submit" disabled={state === 'loading' || !fullName.trim() || !email.trim()}
        ...
        <p className="text-center text-secondary-text text-xs mt-6">
          ...
          <Link href="/wrp/login" className="text-accent hover:underline">Log in here</Link>
        </p>
      </div>
    </div>
  );
